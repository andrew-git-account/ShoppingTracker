...
class LLMService:
    ...
    def extract_statement_transactions(self, pdf_path: str, user_email: str) -> List[Dict]:
        """
        Extract a list of transactions from a bank/card statement PDF (see SP-025).

        Unlike receipt extraction, this reads the PDF's text locally (statements
        are near-universally text-based, generated by the bank's own software,
        not photographed) and sends that text to Claude - not the PDF itself.
        No retry loop: statements have no SP-018-style reconciliation concept.

        Args:
            pdf_path (str): Path to the statement PDF file
            user_email (str): Email of the user whose upload this is (see SP-020) -
                attributed on the logged API call

        Returns:
            List[Dict]: Extracted transactions, each
                {'date': str, 'description': str, 'amount': float, 'currency': str}

        Raises:
            ValueError: If no text could be read from the PDF (e.g. it's scanned/image-only)
            Exception: If the API call fails or the response can't be parsed
        """
        print(f"Extracting transactions from statement: {pdf_path}")

        statement_text = self._extract_pdf_text(pdf_path)
        if not statement_text.strip():
            raise ValueError(
                "Could not read any text from this PDF - it may be a scanned/image-only "
                "statement, which isn't supported yet."
            )

        prompt = self._create_statement_extraction_prompt(statement_text, self.valid_categories)
        response = None

        try:
            response = self.client.messages.create(
                model=self.model,
                max_tokens=8192,  # a statement can list far more records than one receipt's items
                temperature=0.0,
                messages=[
                    {
                        "role": "user",
            ...
            result = self._parse_response(response_text, required_fields=['transactions'])
            self._record_usage(user_email, response.usage, success=True, is_retry=False)

            print(f"Successfully parsed statement with {len(result['transactions'])} transactions")
            return result['transactions']

        except anthropic.APIError as e:
            print(f"Anthropic API error: {e}")
            self._record_usage(user_email, None, success=False, is_retry=False)
            raise Exception(f"Failed to call Claude API: {e}")
        except Exception as e:
            print(f"Unexpected error: {e}")
            self._record_usage(user_email, response.usage if response else None, success=False, is_retry=False)
            raise
    ...
    def _extract_pdf_text(self, pdf_path: str) -> str:
        ...
        Returns:
            str: Extracted text (empty string if the PDF has no extractable
...
Analyze the receipt now: first transcribe the items table row by row, then return the extracted data as JSON:"""
    ...
    def _create_statement_extraction_prompt(self, statement_text: str, valid_categories: List[str] = None) -> str:
        """
        Create the prompt for Claude to extract transactions from statement text.

        Args:
            statement_text (str): Plain text extracted locally from the statement PDF
            valid_categories (List[str]): Categories to offer for transaction
                classification - same vocabulary receipt items use (see SP-025)
        ...
        return f"""You are a bank/credit-card statement data extraction assistant.
Below is the text extracted from a statement PDF. Read through it and identify
every individual transaction (purchase, payment, withdrawal, etc.) listed.

**For each transaction, extract:**
- date: Transaction date in YYYY-MM-DD format
- description: The merchant/payee description exactly as printed
- amount: The transaction amount as a positive number (as a number, not a string)
- currency: ISO 4217 currency code (e.g. "USD", "EUR", "CHF"). If not shown, infer from context or use "USD".
- direction: "debit" if money left the account (a purchase, standing order,
  e-bill, outgoing transfer), or "credit" if money arrived (an incoming
  payment, refund, salary, deposit). This is independent of `amount`, which
  always stays positive either way - `direction` is what tells them apart.
- category: One category from this exact list: {categories_str}
  A statement line has no itemization to categorize from, unlike a receipt -
  make your best guess from the merchant/payee description alone, and use
  "Other" whenever it's genuinely unclear.

**Important guidelines:**
- Include every transaction line, not just a summary or a subset.
- Ignore non-transaction lines: headers, running balances, page totals, marketing text, footers.
- Do not include the statement's own summary/total lines as a transaction.
- `amount` is always a positive number - "how much was this transaction for" -
  regardless of how it's printed (with a minus sign, in parentheses, in a
  separate debit/credit column, etc.). This is not a signed balance change -
  use `direction` for that instead.
- Ensure all number fields are actual numbers, not strings.
- For currency, always return an uppercase ISO 4217 code, never a symbol.
...
Return ONLY the JSON, inside a ```json code block."""
    ...
    def _parse_response(self, response_text: str, required_fields: Optional[List[str]] = None) -> Dict:
        """
        ...
        Args:
            ...
            required_fields (List[str], optional): Top-level keys that must be
                present in the parsed JSON. Defaults to `['items', 'total_amount']`
                (receipt shape) so existing callers need no changes; pass
                `['transactions']` for statement extraction (see SP-025).
        ...
        """
        required_fields = required_fields or ['items', 'total_amount']
        ...
        try:
            ...
            # Ensure any list-shaped field that's required is actually a list
            if 'items' in required_fields and not isinstance(data['items'], list):
                raise ValueError("'items' must be a list")
            if 'transactions' in required_fields and not isinstance(data['transactions'], list):
